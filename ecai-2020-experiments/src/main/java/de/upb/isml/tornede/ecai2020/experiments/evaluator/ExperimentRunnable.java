...
public class ExperimentRunnable implements Runnable {
	...
	public ExperimentRunnable(Experiment experiment, int experimentNumber, int datasetSplit, List<Integer> trainingDatasets, List<Integer> testDatasets, IdBasedRanker ranker, List<Metric> metrics, int numberOfRankingsToTest,
			int amountOfPipelinesToSelect) {
		super();
	...
	public void run() {
		try {
			...
			experiment.runExperiment(datasetSplit, trainingDatasets, testDatasets, ranker, metrics, numberOfRankingsToTest, amountOfPipelinesToSelect);
